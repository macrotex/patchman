...
def get_checksum(checksum_type, data):
...
def refresh_yum_repo(mirror, data, mirror_url, ts):
    ...
    sha = get_checksum(checksum_type, data)
    if sha is None:
        mirror.fail()
        return
...
def checksum_is_valid(computed, provided, mirror):
...
def refresh_gentoo_repo(repo):
    ...
    for mirror in repo.mirror_set.filter(mirrorlist=False, refresh=True):
        ...
        if mirror.last_access_ok:
            ...
            info_message.send(sender=None, text=text)
            md5 = get_checksum('md5', data)
            if not checksum_is_valid(md5, checksum, mirror):
                mirror.fail()
                continue
            else:
                mirror.file_checksum = checksum
